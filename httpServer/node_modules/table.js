...
exports.getUserList=function(req, resp){
    var sql="SELECT * FROM RidingMechanic.user";
    console.log(sql);
    db.excuteSql(sql,function(data, err){
        if(err){
            httpMsg.show500(req,resp,err);
...
exports.addUser=function(req,resp,reqBody){
    try{
        ...
        if(data){
            var sql="Insert INTO RidingMechanic.user(username,password) VALUES("+mysql.escape(data.username)+","+mysql.escape(data.password)+")";
            console.log(sql);
            db.excuteSql(sql,function(data,err){
                if(err){
                    httpMsg.show500(req,resp,err);
    ...
    try{
        ...
        if(data){  
            ...
            if(data.totalAlertSwitch){
                ...
                var sql="UPDATE RidingMechanic.user SET password= ("+mysql.escape(data.password)
                +")WHERE username=("+mysql.escape(data.username)+")";
            }
